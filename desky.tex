%============================================================================
% tento soubor pouzijete pro vysazeni desek
% (c) 2008 Michal Bidlo
% E-mail: bidlom AT fit vutbr cz
% kodovaní: UTF-8 (zmena prikazem iconv, recode nebo cstocs)
%----------------------------------------------------------------------------
% zpracování: make desky, make clean
%============================================================================
% Šablonu upravil: Ing. Jaroslav Dytrych, xdytry00@stud.fit.vutbr.cz
%============================================================================
\documentclass{fitthesis}
%Je-li prace psana v anglickem jazyce, je zapotrebi u tridy pouzit parametr 
%english  \documentclass[english]{fitthesis}

% zde zvolime kodovani, ve kterem je napsan text prace
% "latin2" pro iso8859-2 nebo "cp1250" pro windows-1250
\usepackage{ucs}
\usepackage[utf8x]{inputenc}
\usepackage[T1]{fontenc}
\PrerenderUnicode{ěščřžýáíéĚŠČŘŽÝÁÍÉďťňĎŤŇŮůúÚóÓ}

% nějaké další věci
\pdfpageattr{/Group << /S /Transparency /I true /CS /DeviceRGB>>}

%Informace o praci/projektu
%---------------------------------------------------------------------------
\projectinfo{
  %Prace
  project=DP,               %typ prace BP
  year=2014,                %rok
  date=\today,              %datum odevzdani
  %Nazev prace
  title.cs=Podpora JavaScriptu v zobrazovacím stroji HTML,  %nazev prace v cestine
  title.en=JavaScript Support in an HTML Rendering Engine,       %nazev prace v anglictine
  %Autor
  author=Radim Loskot,      %jmeno prijmeni autora
  author.title.p=Bc.
  %Ustav
  department=UPGM,          % zkratka ustavu
  %Skolitel
  supervisor=Radek Burget, %jmeno prijmeni skolitele
  supervisor.title.p=Ing.,   %titul pred jmenem (nepovinne)
  supervisor.title.a=Ph.D.,    %titul za jmenem (nepovinne)
}


\begin{document}
  % Vysazeni desek
  % ----------------------------------------------
  \makecover
\end{document}
